...
async function generateHashForCommentManager(text) {
    try {
...
class CommentManager {
    ...
    async addComment(newComment) {
        ...
        if (!newComment.selectedTextHash) {
            newComment.selectedTextHash = await generateHashForCommentManager(newComment.selectedText);
        }
...
const DEFAULT_ANNOTATION_CATEGORY_MAPPINGS = [
...
class SideNoteSettingTab extends obsidian_1.PluginSettingTab {
    ...
    async updateAnnotationCategoryMapping(index, patch) {
    ...
    async removeAnnotationCategoryMapping(index) {
        const next = this.plugin.getAnnotationCategoryMappings();
        if (next.length <= 1) {
            new obsidian_1.Notice("至少保留一条分类映射。");
            return;
        }
        next.splice(index, 1);
        this.plugin.settings.annotationCategoryMappings = next;
    ...
    renderAnnotationCategoryMappings(containerEl) {
        ...
        mappings.forEach((mapping, index) => {
            new obsidian_1.Setting(containerEl)
                ...
                .addColorPicker((colorPicker) => colorPicker.setValue(mapping.color).onChange(async (value) => {
                await this.updateAnnotationCategoryMapping(index, { color: value.toLowerCase() });
            }))
                .addText((text) => text.setPlaceholder(FALLBACK_CATEGORY).setValue(mapping.category).onChange(async (value) => {
        ...
        new obsidian_1.Setting(containerEl)
            ...
            .addButton((button) => button.setButtonText("新增映射").onClick(async () => {
            const next = this.plugin.getAnnotationCategoryMappings();
            next.push({
    ...
    display() {
        ...
        new obsidian_1.Setting(containerEl).setName("Annotation path strategy").setDesc("Choose whether generated annotation notes only group by source key, or also mirror the source note folder tree.")
        ...
        new obsidian_1.Setting(containerEl).setName("Sync current file comments and annotations").addButton((button) => button.setButtonText("Sync Now").onClick(async () => {
...
class SideNote extends obsidian_1.Plugin {
    ...
    async areCommentsSemanticallyEqual(left, right) {
    ...
    getDefaultAnnotationCategoryMappings() {
        return DEFAULT_ANNOTATION_CATEGORY_MAPPINGS.map((mapping) => ({ ...mapping }));
    }
    sanitizeAnnotationCategoryMappings(rawMappings) {
        ...
        return rawMappings.map((rawMapping, index) => {
            ...
            const candidate = (rawMapping && typeof rawMapping === "object") ? rawMapping : {};
    ...
    getAnnotationCategoryMappings() {
        return this.sanitizeAnnotationCategoryMappings(this.settings.annotationCategoryMappings);
    }
    getPresetAnnotationColors() {
        return this.getAnnotationCategoryMappings().map((mapping) => ({
            name: mapping.category,
            value: mapping.color,
        }));
    }
    getAnnotationPathStrategy() {
        return this.settings.annotationPathStrategy === "mirror_source_tree"
            ? "mirror_source_tree"
            : "group_by_source_key";
    }
    getAnnotationOutputRoot() {
        return (0, obsidian_1.normalizePath)(this.settings.annotationOutputFolder?.trim() || DEFAULT_SETTINGS.annotationOutputFolder);
    }
    getSourceRelativeParentPath(file) {
        const parentPath = file.parent?.path;
        if (!parentPath || parentPath === "/")
            return "";
        return (0, obsidian_1.normalizePath)(parentPath);
    }
    ...
    getManagedAnnotationMetadata(file) {
        ...
        if (frontmatter?.type !== "annotation")
            return null;
        if (!annotationId.startsWith("side-comments:"))
            return null;
    ...
    parseAnnotationBacklinkComment(content) {
    ...
    async evaluateAnnotationCommentSyncState(currentContent, existingFrontmatter, jsonCommentText) {
    ...
    buildAnnotationSyncSummaryMessage(created, updated, unchanged, blockedByReason) {
    ...
    async syncCommentsToAnnotationsForFile(file) {
        ...
        new obsidian_1.Notice(this.buildAnnotationSyncSummaryMessage(created, updated, unchanged, blockedByReason));
    }
    async syncAnnotationNoteToCommentJson(file) {
        if (!(file instanceof obsidian_1.TFile) || file.extension !== "md") {
            new obsidian_1.Notice("No active Markdown file found.");
            return;
        }
        ...
        if (!metadata) {
            new obsidian_1.Notice("Current file is not a managed Side Comments annotation note.");
            return;
        }
        await this.reloadCommentsFromDisk();
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : "Unknown parse error.";
            new obsidian_1.Notice(`批注回写已阻止：${message}`);
            return;
        }
        const targetComment = this.comments.find((comment) => comment.timestamp === metadata.timestamp);
        if (!targetComment) {
            new obsidian_1.Notice("批注回写已阻止：未找到对应的 JSON comment。");
            return;
        }
        ...
        if (nextCommentHash === currentCommentHash) {
            new obsidian_1.Notice("当前 annotation 与 JSON comment 在规范化后内容一致，无需回写。");
            return;
        }
        ...
        if (!syncState.storedHash) {
            new obsidian_1.Notice("批注回写已阻止：缺少 comment_hash_at_sync baseline，无法安全覆盖。");
            return;
        }
        if (syncState.conflict || syncState.jsonChangedSinceSync) {
            new obsidian_1.Notice("批注回写已阻止：JSON comment 自上次同步后已变化，请先重新同步或人工确认。");
            return;
        }
        ...
        new obsidian_1.Notice("批注回写完成：JSON comment 已更新。");
    }
    async syncCurrentFileCommentsAndAnnotations(file) {
        const metadata = this.getManagedAnnotationMetadata(file);
        if (metadata) {
    ...
    async onload() {
        ...
        this.registerEvent(this.app.vault.on('modify', async (file) => {
            ...
            if (file.path.startsWith(dataFolder + '/')) {
                if (file instanceof obsidian_1.TFile && file.extension === 'json') {
                    try {
                        await this.reloadCommentsFromDisk();
                        this.refreshViews();
                        this.refreshEditorDecorations();
...
module.exports.default = SideNote;